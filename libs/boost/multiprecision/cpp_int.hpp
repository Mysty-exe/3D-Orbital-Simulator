      ...
      namespace backends
      {
         ...
         public:
            ...
            static constexpr std::size_t internal_limb_count =
                ...
                : (sizeof(limb_data) / sizeof(limb_type)) > 1 ? (sizeof(limb_data) / sizeof(limb_type))
                                                              : 2;
         ...
         public:
            ...
            BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
                boost::multiprecision::detail::is_unsigned<A>::value && trivial_tag::value, cpp_int_backend &>::type
            operator=(const A &val) noexcept(noexcept(std::declval<cpp_int_backend>().check_in_range(std::declval<A>())))
            {
            ...
            template <class A>
            BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
                !(boost::multiprecision::detail::is_unsigned<A>::value || !boost::multiprecision::detail::is_integral<A>::value) && trivial_tag::value, cpp_int_backend &>::type
            operator=(const A &val) noexcept(noexcept(std::declval<cpp_int_backend>().check_in_range(std::declval<A>())) && noexcept(std::declval<cpp_int_backend>().sign(true)))
            {
               this->check_in_range(val);
            ...
            template <class A>
            BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
                std::is_convertible<A, limb_type>::value && !boost::multiprecision::detail::is_integral<A>::value && trivial_tag::value, cpp_int_backend &>::type
         ...
         private:
            ...
            BOOST_MP_CXX14_CONSTEXPR void do_assign_float(F a)
            {
               ...
               BOOST_MP_FLOAT128_USING using std::floor;
               using std::frexp;
               using std::ldexp;
         ...
         private:
            ...
            std::string do_get_trivial_string(std::ios_base::fmtflags f, const std::integral_constant<bool, true> &) const
            {
               ...
               if (base == 8 || base == 16)
               {
                  ...
                  {
                     const char *pp = base == 8 ? "0" : (f & std::ios_base::uppercase) ? "0X"
                                                                                       : "0x";
